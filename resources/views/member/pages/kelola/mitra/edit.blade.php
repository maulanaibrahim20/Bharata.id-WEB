    ...
    <div class="p-4 sm:ml-64">
        <div class="rounded-lg mt-14">
            <div class="bg-white rounded-lg shadow p-6">
                ...
                @endif
                <form action="{{ route('mitra.update', $mitra->id) }}" enctype="multipart/form-data" method="post"
                    class="space-y-4 md:space-y-6 needs-validation" novalidate>
                    @csrf
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                            for="validationCustom011">Nama Mitra</label>
                        <input type="text"
                            ...
                            id="validationCustom011" name="name" value="{{ $mitra->name }}" required>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                        ...
                        <textarea name="description" style="resize: none"
                            class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 w-full px-1 py-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            id="validationCustom12" required>{{ $mitra->description }}</textarea>

                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                            for="validationCustom12">Lokasi</label>
                        <input type="text"
                            ...
                            name="location" data-bs-height="180" value="{{ $mitra->location }}"  />
                    </div>
                    <div class="flex items-center gap-2">
                        <a href="{{ url('/member/kelola/mitra') }}"
